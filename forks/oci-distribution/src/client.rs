...
impl TryFrom<ClientConfig> for Client {
  ...
  fn try_from(config: ClientConfig) -> Result<Self, Self::Error> {
    let mut client_builder =
      reqwest::Client::builder().danger_accept_invalid_certs(config.accept_invalid_certificates);
...
impl Client {
  ...
  pub async fn pull(
    ...
    self
      .validate_layers(&manifest, accepted_media_types)
      .await?;
  ...
  async fn auth(
    ...
    let auth = WwwAuthenticate::parse_header::<Raw>(&dist_hdr.as_bytes().into())?;
  ...
  async fn _pull_manifest_and_config(
    ...
    self
      .pull_layer(image, &manifest.config.digest, &mut out)
      .await?;
  ...
  async fn push_config(
    ...
    let (end_location, _) = self
      ...
      .await?;
    self
      .end_push_session(&end_location, image, config_digest)
      .await
  }
...
mod test {
  ...
  fn test_to_v2_blob_url() {
    ...
    let blob_url =
      Client::default().to_v2_blob_url(image.registry(), image.repository(), "sha256:deadbeef");
    assert_eq!(
      blob_url,
  ...
  fn can_generate_valid_digest() {
    ...
    let combination_hash = sha256_digest(&combination.into_iter().flatten().collect::<Vec<u8>>());
