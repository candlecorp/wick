...
impl Dispatch for Dispatcher {
  fn dispatch(op: &str, payload: &[u8]) -> CallResult {
    ...
    let result = match op {
      "byte-count" => crate::components::generated::byte_count::Component::default().execute(&payload),
  ...
  pub mod main {
    ...
    pub fn populate_inputs(mut payload: TransportMap) -> Result<Inputs, TransportError> {
      Ok(Inputs {
        ...
        network: payload.consume("network")?,
      })
    }
    ...
    fn populate_inputs(payload: &IncomingPayload) -> Result<Inputs, WasmError> {
      Ok(Inputs {
        ...
        network: deserialize(payload.get("network")?)?,
      })
    }
    ...
    impl From<Inputs> for TransportMap {
      fn from(inputs: Inputs) -> TransportMap {
        ...
        map.insert("network", MessageTransport::success(&inputs.network));
        map
      }
    }
