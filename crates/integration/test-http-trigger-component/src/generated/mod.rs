...
pub use wick_component::{packet as wick_packet, runtime, wasmrs, wasmrs_codec, wasmrs_rx, Bytes};
#[allow(unused)]
...
extern "C" fn __wasmrs_init(guest_buffer_size: u32, host_buffer_size: u32, max_host_frame_len: u32) {
  ...
  wick_component::wasmrs_guest::register_request_channel(
    "wick",
    "http_handler",
    Box::new(Component::http_handler_wrapper),
  );
}
#[cfg(target_family = "wasm")]
...
#[doc = "Types associated with the `http_handler` operation"]
pub mod http_handler {
  ...
  pub struct Outputs {
    #[allow(unused)]
    pub(crate) body: wick_packet::Output<wick_component::Bytes>,
...
#[cfg(target_family = "wasm")]
pub trait HttpHandlerOperation {
  ...
  async fn http_handler(
    ...
    body: WickStream<wick_component::Bytes>,
...
impl Component {
  ...
  > {
    ...
    let outputs = http_handler::Outputs::new(channel.clone());
    runtime::spawn("http_handler_wrapper", async move {
      let (config, request, body) = wick_component :: payload_fan_out ! (input , raw : false , Box < dyn std :: error :: Error + Send + Sync > , http_handler :: Config , [("request" , types :: http :: HttpRequest) , ("body" , wick_component :: Bytes) ,]);
