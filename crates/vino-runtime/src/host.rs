...
impl Host {
    ...
    pub async fn apply_manifest(&self, manifest: HostManifest) -> Result<()> {
        ...
        for msg in crate::manifest::generate_schematic_messages(&manifest).await {
            hc.send(msg).await??;
        }
    ...
    /// Request the output from a schematic
    pub async fn request(
        &self,
        schematic: String,
        data: HashMap<String, impl Serialize>,
    ) -> Result<HashMap<String, MessagePayload>> {
        self.ensure_started()?;

        let target = WasmCloudEntity::Schematic(schematic.to_string());
        ...
        let bus = MessageBus::from_hostlocal_registry(&self.id);
        trace!("Invoking schematic '{}'", schematic);
        let result: InvocationResponse = bus
            .send(Invocation::new(
                &self.kp,
                WasmCloudEntity::Actor(SYSTEM_ACTOR.to_string()),
                target,
                "system::request",
                MessagePayload::MultiBytes(serialized_data),
            ))
            .await?;
        ...
        if let Some(e) = result.error {
            ...
            let outputs: HashMap<String, MessagePayload> = deserialize(&result.msg)?;
...
mod test {
    ...
    use crate::{deserialize, Host, HostBuilder, HostManifest, Result};
    ...
    async fn get_connected_host() -> Result<Host> {
        let h = HostBuilder::new()
            .with_namespace("wasmcloud-host::test")
            .oci_allow_latest()
    ...
    #[actix_rt::test]
    async fn basic_schematic() -> crate::Result<()> {
        let host = get_connected_host().await?;
        let yaml = "---
actors:
  - ./test/fixtures/validate.wasm #MDZNW2K7QBTK23OP4HO4BGPXKRNG3IGJW47MZMAKLOZYM72MDGQ5VIAL
schematics:
  - name: test
    components:
      validate_password:
        ref: MDZNW2K7QBTK23OP4HO4BGPXKRNG3IGJW47MZMAKLOZYM72MDGQ5VIAL
      log:
        ref: vino::log
    connections:
      - from:
...
";
        ...
        host.apply_manifest(manifest).await?;
        ...
        let data: HashMap<String, String> =
            vec![("input_port1".to_string(), "test_input".to_string())]
                .iter()
                .cloned()
                .collect();
        let result = host.request("test".to_string(), data).await?;
        trace!(
        ...
        trace!("result: {:?}", result);
        let output1: &MessagePayload = result.get("output1").unwrap();
        if let MessagePayload::Bytes(bytes) = output1 {
            let output1: String = deserialize(&bytes)?;
            assert_eq!(output1, "test_input");
        } else {
            panic!();
        }
...
";
        ...
        trace!("requesting schematic execution");
        let time = Instant::now();
        let data: HashMap<String, String> = vec![("input_port1".to_string(), "short".to_string())]
            .iter()
            .cloned()
            .collect();
        let result = host.request("test".to_string(), data).await?;
        ...
        trace!("result: {:?}", result);
        let output1: &MessagePayload = result.get("output1").unwrap();
        if let MessagePayload::Exception(string) = output1 {
            assert_eq!(string, "Password needs to be longer than 8 characters");
        } else {
            panic!();
        }
        Ok(())
    ...
    #[actix_rt::test]
    async fn multiple_inputs() -> crate::Result<()> {
        let host = get_connected_host().await?;
        let yaml = "---
schematics:
  - name: test
...
";
        ...
        let data: HashMap<String, u64> =
            vec![("left".to_string(), 42), ("right".to_string(), 302309)]
                .iter()
                .cloned()
                .collect();
        let result = host.request("test".to_string(), data).await?;
...
";
        ...
        let data: HashMap<String, u64> =
            vec![("left".to_string(), 42), ("right".to_string(), 302309)]
                ...
                .cloned()
                .collect();
        let result = host.request("first_schematic".to_string(), data).await?;
        trace!(
            "result: {:?} took {} microseconds",
            result,
            time.elapsed().as_micros()
        ...
        let time = Instant::now();
        let data: HashMap<String, String> = vec![("input".to_string(), "some string".to_string())]
            .iter()
            .cloned()
            .collect();
        let result = host.request("second_schematic".to_string(), data).await?;
        trace!(
            "result: {:?} took {} microseconds",
            result,
            time.elapsed().as_micros()
        ...
        let output: &MessagePayload = result.get("output").unwrap();
        if let MessagePayload::Bytes(data) = output {
            let result: String = deserialize(data)?;
