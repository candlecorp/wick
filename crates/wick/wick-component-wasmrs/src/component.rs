...
use crate::wasmrs_host::{SetupPayload, WasmHost, WasmHostBuilder};
use crate::Error;
...
pub struct ComponentSetup {
  ...
  #[builder(setter(), default)]
  pub callback: Option<LocalScope>,
...
impl Component for WasmrsComponent {
  fn handle(
    &self,
...
mod tests {
  ...
  async fn load_component() -> Result<WasmrsComponent> {
    ...
    let setup = ComponentSetupBuilder::default()
      ...
      .callback(Some(LocalScope::new(Arc::new(|_, _, _, _, _, _| {
        Box::pin(async { Ok(packet_stream!(("test", "test"))) })
      }))))
      .build()?;

    let c = WasmrsComponent::try_load("test", file, setup, Span::current()).await?;
    Ok(c)
  }
  ...
  async fn test_component_error() -> Result<()> {
    ...
    let outputs = component
      .handle(invocation, Some(config.try_into()?), Default::default())
      .await?;
    debug!("got stream");
