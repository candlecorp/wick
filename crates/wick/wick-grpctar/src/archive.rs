...
pub fn make_archive<T: Read>(
  ...
  let claims = wick_wascap::build_collection_claims(signature.clone(), subject_kp, issuer_kp, claims_options);
  ...
  let jwt_bytes = wick_wascap::make_jwt(combined_bytes, &claims, issuer_kp)?;
...
pub fn validate_collection<B: Read, I: Read>(binary: B, interface: I, jwt: Vec<u8>) -> Result<(), ParError> {
  ...
  let token = wick_wascap::decode_token(jwt)?;
  let hash = wick_wascap::hash_bytes(combined)?;
  trace!(hash = %hash, ?token, "oci archive");

  wick_wascap::assert_valid_jwt(&token, &hash)?;
  Ok(())
}
