...
pub(crate) struct GenerateCommon {
  /// Location of key files for signing. Defaults to $WAFL_KEYS ($HOME/.vino/keys or %USERPROFILE%/.vino/keys on Windows).
...
/// Retrieves a keypair by name in a specified directory, or $WAFL_KEYS ($HOME/.vino/keys) if directory is not specified.
...
/// Lists all keypairs (file extension .nk) in a specified directory or $WAFL_KEYS($HOME/.vino/keys) if directory is not specified.
