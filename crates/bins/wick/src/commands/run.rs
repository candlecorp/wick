...
use wick_runtime::configuration::{ApplicationContext, Channel};
...
pub(crate) async fn handle_command(opts: RunCommand) -> Result<()> {
  ...
  let app_config = wick_runtime::configuration::load_yaml(&opts.location)?;
  ...
  for (name, channel_config) in app_config.channels {
    ...
    match wick_runtime::configuration::get_channel_loader(&channel_config.uses) {
