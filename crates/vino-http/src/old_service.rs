  ...
  S::Error: Into<BoxError> + Send,
{
  ...
  fn call(&mut self, req: Request<Body>) -> Self::Future {
    match RequestKind::new(req.headers(), req.method(), req.version()) {
      ...
      RequestKind::GrpcWeb { .. } => {
        trace!("simple:method not allowed:{}", req.method());
      ...
      RequestKind::GrpcWebPreflight {
        ...
        Ok(headers) => {
          trace!("preflight:{}", req.uri().path(),);
          self.no_content(headers)
        }
        Err(e) => {
          trace!("preflight:error:{}", e);
          self.response(StatusCode::FORBIDDEN)
        }
      },
      ...
      RequestKind::Other(Version::HTTP_2) => {
        debug!("other h2:{:?}", req.headers().get(header::CONTENT_TYPE));
        Box::pin(self.inner.call(req))
      }

      // Return HTTP 400 for all other requests.
      RequestKind::Other(_) => {
        debug!("other h1:{:?}", req.headers().get(header::CONTENT_TYPE));
...
fn coerce_request(mut req: Request<Body>, encoding: Encoding) -> Request<Body> {
  ...
  req
    .map(|b| GrpcWebCall::request(b, encoding))
