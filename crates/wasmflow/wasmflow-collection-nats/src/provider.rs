...
impl RpcHandler for Provider {
  ...
  fn get_list(&self) -> RpcResult<Vec<wasmflow_interface::HostedType>> {
    let components =
      block_on(self.mesh.list_components(self.mesh_id.clone())).map_err(|e| RpcError::ProviderError(e.to_string()))?;
...
mod tests {
  ...
  async fn integration_test_component() -> TestResult<()> {
    ...
    mesh.handle_namespace(ns.to_owned(), get_provider()).await?;

    let provider = Provider::new(ns.to_owned(), Arc::new(mesh)).await?;
