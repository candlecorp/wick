...
#[instrument(skip(provider, opts))]
pub(crate) async fn initialize_mesh_provider(
  provider: &ProviderDefinition,
  namespace: String,
  opts: ProviderInitOptions,
) -> ProviderInitResult {
  ...
  let mesh = match opts.mesh {
    Some(mesh) => mesh,
    None => {
      return Err(ProviderError::Mesh(
        "Mesh provider defined but no mesh available".to_owned(),
      ))
    }
  };

  let provider = Arc::new(wasmflow_collection_nats::provider::Provider::new(provider.reference.clone(), mesh).await?);
