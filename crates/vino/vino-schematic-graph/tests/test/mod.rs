...
use vino_schematic_graph::{ComponentReference, Network, PortDirection, Schematic};
...
pub fn load<T: AsRef<Path>>(path: T) -> Result<vino_manifest::HostManifest> {
  Ok(vino_manifest::HostManifest::load_from_file(path.as_ref())?)
}

pub fn from_manifest(network_def: &vino_manifest::NetworkDefinition) -> Result<Network<Value>> {
