...
TEMPLATE="${SCRIPT_DIR}/template.hbs"
...
use crate::provider_trait::VinoProviderComponent;
pub(crate) mod generated;
"

GET_NATIVE_ACTOR_FN="
pub(crate) fn get_native_component(
    name: &str,
) -> Option<Box<dyn VinoProviderComponent>> {
  match name {
"
...
for WIDL_PATH in ${SCRIPT_DIR}/*.widl; do
  ...
  vino-codegen rust $WIDL_PATH -t provider-component | rustfmt --quiet >$FS_MODULE_PATH
  echo "pub(crate) mod ${FS_MODULE_NAME};" >>$GENERATED_MODULE_FILE
  MOD_LINES="$MOD_LINES"$'\n'"pub(crate) mod ${FS_MODULE_NAME};"
  GET_NATIVE_ACTOR_FN="$GET_NATIVE_ACTOR_FN"$'\n'"\"vino::${MODULE_NAME}\" => Some(Box::new(${FS_MODULE_NAME}::Component::default())),"
