...
pub(crate) fn postgres_row_to_json_value(row: &Row) -> Result<JSONValue, Error> {
  let row_data = postgres_row_to_row_data(row)?;
  Ok(JSONValue::Object(row_data))
}

pub(crate) type JSONValue = serde_json::Value;
pub(crate) type RowData = Map<String, JSONValue>;

pub(crate) fn postgres_row_to_row_data(row: &Row) -> Result<RowData, Error> {
  let mut result: Map<String, JSONValue> = Map::new();
  for (i, column) in row.columns().iter().enumerate() {
    let name = column.name();
    let json_value = pg_cell_to_json_value(&row, column, i)?;
    result.insert(name.to_owned(), json_value);
  }
  Ok(result)
}

pub(crate) fn pg_cell_to_json_value(row: &Row, column: &Column, column_i: usize) -> Result<JSONValue, Error> {
  let f64_to_json_number = |raw_val: f64| -> Result<JSONValue, Error> {
    let temp = serde_json::Number::from_f64(raw_val.into()).ok_or(Error::InvalidFloat)?;
...
fn get_basic<'a, T: FromSql<'a>>(
  ...
  let raw_val = row
    ...
    .map_err(|e| Error::GetBasic(column.name().to_owned(), e))?;
...
fn get_array<'a, T: FromSql<'a>>(
  ...
  let raw_val_array = row
    ...
    .map_err(|e| Error::GetArray(column.name().to_owned(), e))?;
