...
extern "C" fn __wasmrs_init(guest_buffer_size: u32, host_buffer_size: u32, max_host_frame_len: u32) {
  ...
  wick_component::wasmrs_guest::register_request_channel("wick", "request", Box::new(Component::request_wrapper));
}
#[cfg(target_family = "wasm")]
mod provided {
  #[allow(unused)]
  ...
  impl From<PostOpConfig> for wick_packet::RuntimeConfig {
    fn from(v: PostOpConfig) -> Self {
  ...
  impl ClientComponent {
    ...
    pub fn post_op(
      &self,
      op_config: PostOpConfig,
      id: impl wick_component::Stream<Item = Result<String, wick_component::AnyError>> + 'static,
      name: impl wick_component::Stream<Item = Result<String, wick_component::AnyError>> + 'static,
    ) -> std::result::Result<
      (WickStream<types::http::HttpResponse>, WickStream<wick_component::Value>),
      wick_packet::Error,
    > {
      let id = id.map(wick_component::wick_packet::into_packet("id"));
      let name = name.map(wick_component::wick_packet::into_packet("name"));
      let stream = wick_component::empty();
      let stream = stream.merge(id).merge(name).chain(wick_component::iter_raw(vec![
        Ok(Packet::done("id")),
        Ok(Packet::done("name")),
      ]));
      let stream = wick_packet::PacketStream::new(Box::pin(stream));
      let mut stream = self.post_op_raw(op_config, stream)?;
      Ok(wick_component::payload_fan_out!(
          stream, raw : false, wick_component::AnyError, [("response",
          types::http::HttpResponse), ("body", wick_component::Value)]
      ))
    }
    #[allow(unused)]
    pub fn post_op_raw<T: Into<wick_packet::PacketStream>>(
      &self,
      op_config: PostOpConfig,
      stream: T,
    ) -> std::result::Result<wick_packet::PacketStream, wick_packet::Error> {
      Ok(self.component.call(
        "post_op",
        stream.into(),
        Some(op_config.into()),
        self.inherent.clone().into(),
      )?)
    }
  }
}
...
mod provided_wasm {
    ...
    T: std::fmt::Debug,
  {
    fn provided(&self) -> Provided {
      ...
      Provided {
        client: ClientComponent::new(config.provided.get("client").cloned().unwrap(), inherent.clone()),
      }
    }
  }
}
...
///Types associated with the `request` operation
pub mod request {
  #[allow(unused)]
  use super::*;
  #[derive(Debug, Clone, Default, ::serde::Serialize, ::serde::Deserialize, PartialEq)]
  ...
  pub struct Outputs {
    #[allow(unused)]
    pub(crate) output: wick_packet::OutgoingPort<wick_component::Value>,
  }
  ...
  pub trait Operation {
    ...
    #[allow(unused)]
    async fn request(
      id: WickStream<String>,
      name: WickStream<String>,
...
impl Component {
  fn request_wrapper(
    ...
    let outputs = request::Outputs::new(channel.clone());
    runtime::spawn("request_wrapper", async move {
      #[allow(unused_parens)]
      let (config, (id, name)) = wick_component::payload_fan_out!(
          input, raw : false, wick_component::AnyError, request::Config,
      ...
      if let Err(e) = Component::request(Box::pin(id), Box::pin(name), outputs, config).await {
